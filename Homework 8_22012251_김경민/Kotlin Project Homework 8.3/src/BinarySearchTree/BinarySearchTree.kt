...
class BinarySearchTree<E> {
    ...
    private fun _insertInOrder(
        subRoot_bstn: BinarySearchTreeNode<E>?,
        pr_bstn: BinarySearchTreeNode<E>?,
        newEntry: E
    ): BinarySearchTreeNode<E>? {
        ...
        return if (subRoot_bstn == null) {
            ...
            if (_num_entry == 0) _root = new_bstn
// this BSTN becomes the root
...
// implement recursive function for search with in-order-traversal
    ...
    fun printBST_withDepth() {
        ...
        println(str); _printBST_withDepth (_root, 0)
    }
    ...
    fun _reBalance(curSubRoot: BinarySearchTreeNode<E>): BinarySearchTreeNode<E>? {
        ...
        if (heightDiff > 1) { // left subtree is higher
            ...
            } else { _rotateLR(curSubRoot)
            }
        } else if (heightDiff < -1) {
            newSubRoot = if (_getHeightDiff(curSubRoot.getRightChild()) < 0) { _rotateRR(curSubRoot)
            } else { _rotateRL(curSubRoot)
            }
        }
        return newSubRoot
    }
    fun _insertAndRebalance(
        curSubRoot: BinarySearchTreeNode<E>?,
        curParent: BinarySearchTreeNode<E>?, newEntry: E
    ): BinarySearchTreeNode<E>? {
    ...
    fun insert_withRebalancing(newEntry: E) {
        ...
        if (newSubRoot != null) _root = newSubRoot
    }
}