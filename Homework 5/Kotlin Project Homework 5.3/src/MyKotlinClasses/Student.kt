...
class Student (nm: String, rgID: Int, var stID : Int, var stMajor: String): Person(nm, rgID), I_StudentActivity {
    ...
    // I_StudentActivity 인터페이스 구현
    override fun talk() {
        println("Student ($name) is talking")
    }

    override fun study() {
        println("Student ($name) is stduying")
    }
}