...
class WeightedGraph {
        ...
        for (i in 0 until num_vrtx) {
            for (j in 0 until num_vrtx) {
                if (i == j)
                    ...
                    distTbl[i][j] = Int.MAX_VALUE / 2 // 정수 오버플로 방지
            }
        }
    ...
    fun printDistTable() {
        ...
        print(" |")
        for (i in 0 until num_vrtx) {
            System.out.printf("%5s", vrtxArray[i].getName()) }
        print("\n-----+")
        for (i in 0 until num_vrtx) {
            print("-----") }
        println()
    ...
    private fun _DFS(cur_start: Vertex, dest: Vertex, path: MutableList<Vertex>) {
        ...
        for (v in neighbors) {
            ...
            if (!dest_arrived) {
                _DFS(v, dest, path)
                if (!dest_arrived) {
                    path.removeAt(path.size - 1) // 마지막으로 추가된 vertex 제거
                }
            }
            ...
            for (v in vrtxArray) {
                if (v.visited === true && v.level === searchLevel) {
                    for (w in adjList[v.getVID()]) {
                        if (w.visited === false) { w.visited = true // set as Visited
    ...
    fun DijkstraShortestPath(start: Vertex, dest: Vertex): List<Vertex?> {
        ...
        while (!remainVrtxs.isEmpty()) {
            minAccDist = Int.MAX_VALUE / 2 // 정수 오버플로 방지
            minVID = -1
            minVrtx = null
            ...
            for (v in remainVrtxs) {
                /* edge relaxation */
                if (v!!.accDist > minAccDist + distTbl[minVID][v.vid]) { v.accDist = minAccDist + distTbl[minVID][v.vid]
                    v.prev = minVrtx
                }
            }
            if (minVrtx.equals(dest)) { // 목적지에 도착한 경우
                break
            }
        }
        path.clear()
    ...
    fun DAGShortestPath(vrtx_start: Vertex?, vrtx_dest: Vertex): List<Vertex>? {
        ...
        for (v in vrtxArray) {
            if (v.equals(vrtx_start)) {
                v.accDist = 0
                v.prev = vrtx_start
            } else {
                v.accDist = Int.MAX_VALUE / 2
                v.prev = null
            }
            v.visited = false
        }
        _DAG_traverse(vrtx_start)
        /* for debugging purpose */System.out.printf("Accumulated dist : \n")
        for (v in vrtxArray) {
            print("(%s:%3d, prev:%s) ".format(v.vname, v.accDist, v.prev!!.getName()))
        }
        println()
        path.clear()
        path.add(vrtx_dest)
        var v: Vertex? = vrtx_dest
        while (!v!!.equals(vrtx_start)) {
            v = v.prev
            if (v != null) {
                path.add(0, v)
            }
